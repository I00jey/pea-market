...
async function withdraw() {
    ...
    await axios({
        ...
        if (res.data.result) {
            swal(
                "회원탈퇴 성공",
                form.id.value + "님 회원탈퇴가 완료 되었습니다",
                "success"
            );
            document.location.href = "/";
        } else {
...
function userInfoEdit() {
    ...
    if (
        form.newpw.value.length != 0 &&
        form.newpw.value.length != form.newpwchk.value.length
    ) {
        swal(
            "회원 정보 수정 실패",
            "변경할 비밀번호 정보가 일치하지 않습니다.",
