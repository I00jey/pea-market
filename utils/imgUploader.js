...
function deleteProfileImg(bucketName, fileNameToDelete) {
    const filename = fileNameToDelete.substring(
        fileNameToDelete.lastIndexOf("/") + 1
    );
    const params = {
        Bucket: bucketName,
        Key: filename,
    ...
    return new Promise((resolve, reject) => {
        s3.deleteObject(params, (err, data) => {
            if (err) {
                reject(
                    `Error deleting object ${fileNameToDelete} from ${bucketName}: ${err}`
                );
            } else {
                resolve(
                    `Object ${fileNameToDelete} deleted successfully from ${bucketName}.`
                );
            }
        });
    });
